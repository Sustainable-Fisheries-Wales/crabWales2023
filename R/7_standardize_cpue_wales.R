...
# read in data
observer_data_crab <- readr::read_csv("processed_data/wales/observer_data_crab_nominal.cpue_potset.csv",
...
cpue_data_crab <- observer_data_crab |> 
...
# density covaraites
covariate_data <- observer_data_crab |>
  dplyr::filter(!is.na(potset)) |> dplyr::filter(ices_rect!="NANANA") |>
  dplyr::mutate(areaSwept_km2 = 1) |>
  dplyr::arrange(year, month) |>
...
catchability_data <- observer_data_crab |>
  dplyr::filter(!is.na(potset)) |> dplyr::filter(ices_rect!="NANANA") |>
  dplyr::mutate(areaSwept_km2 = 1) |>
  dplyr::arrange(year, month) |>
...
run_dir <- paste0(getwd(),"processed_data/wales/vast/crab")
...
abundance_index <- readr::read_csv("processed_data/wales/vast/crab/index.csv") 
...
# export the file
readr::write_csv(abundance_index, file = "processed_data/wales/observer_data_crab_wales_abundance.index_ss.csv") 
