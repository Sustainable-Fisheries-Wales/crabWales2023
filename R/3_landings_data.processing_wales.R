...
effort_wales <- effort |>
  ...
  dplyr::left_join(ices_rec_wales, by = c("ices_id"="ICES Rectangle")) #|> # NEED TO AGGREGATE BY ICES RECTANGLE

# all gear types 
effort_wales_select <- effort_wales |> dplyr::select("year", "objectid", "ices_id", "sub_rec", "id", "gndkwh", "genkwh", "gnkwh", "gnckwh", "gnskwh", "gtrkwh",
                                                     "tbnkwh", "tmkwh", "tmskwh", "txkwh", "otkwh", "otbkwh", "tbkwh", "tbbkwh", "otmkwh", "ottkwh", "ptkwh", "ptbkwh",  "ptmkwh", "fixkwh", "fpokwh")
effort_wales_select_long <- effort_wales_select |> 
  tidyr::gather(gear, effort, gndkwh:fpokwh, factor_key = TRUE) |> 
  dplyr::mutate(gear = dplyr::case_when(gear %in% c("gndkwh", "genkwh", "gnkwh", "gnckwh", "gnskwh", "gtrkwh") ~ "Gill nets and entangling nets",
                                        gear %in% c("fixkwh", "fpokwh") ~ "Traps",
                                        gear %in% c("tbnkwh", "tmkwh", "tmskwh", "txkwh", "tbkwh", "tbbkwh", "otkwh", "otbkwh", "otmkwh", "ottkwh", "ptkwh", "ptbkwh", "ptmkwh") ~ "Trawls")) 
...
# subset by species
landings_vessel_over15_gear_effort_crab <- landings_vessel_over15_gear |>
  dplyr::filter(year <= 2020) |>
  dplyr::left_join(effort_wales_select_ices.rec, by = c("gear_cat"="gear", "ices_rectangle"="ices_id", "year")) |>
  dplyr::filter(species_name == "Crabs (C.P.Mixed Sexes)") |>
  dplyr::mutate(nominal.cpue = landings/effort)

# export cpue estimates
readr::write_csv(landings_vessel_over15_gear_effort_crab, file = "processed_data/wales/ifish_cpue_over15_wales_crab.csv") 
...
allmyData1 <- allmyData1 |> dplyr::select(year, month, vessel_nationality, fao_area, ices_division, rectangle, length_group, gear_category, species, species_group, 
...
ifish_landings_wales_crab <- ifish_landings_wales |> 
...
ifish_landings_wales_crab_bycatch <- ifish_landings_wales |> 
  dplyr::filter(species %in% c("Crabs", "Crabs (C.P.Mixed Sexes)")) |>  
  dplyr::filter(!(gear_category == "Pots and traps")) 

# export output as csv
readr::write_csv(ifish_landings_wales_crab, file = "processed_data/wales/ifish_landings_wales_crab_clean.csv")
readr::write_csv(ifish_landings_wales_crab_bycatch, file = "processed_data/wales/ifish_landings_wales_crab_bycatch_clean.csv")

# aggregate by year
ifish_landings_wales_crab_annual <- ifish_landings_wales_crab |> 
  dplyr::group_by(species, year, lon, lat) |> 
  dplyr::summarise(landings = sum(live_weight_tonnes), 
                   econ.value = sum(value_pounds)/1000000)

ifish_landings_wales_crab_bycatch_annual <- ifish_landings_wales_crab_bycatch |> 
  dplyr::group_by(species, year, lon, lat) |> 
  dplyr::summarise(landings = sum(live_weight_tonnes), 
                   econ.value = sum(value_pounds)/1000000)

# export output as csv
readr::write_csv(ifish_landings_wales_crab_annual, file = "processed_data/wales/ifish_landings_wales_crab_annual_clean.csv")
readr::write_csv(ifish_landings_wales_crab_bycatch_annual, file = "processed_data/wales/ifish_landings_wales_crab_bycatch_annual_clean.csv")
...
(plot2 <- ggplot2::ggplot() +  
  ...
  ggplot2::facet_wrap(~ year, strip.position = "top", ncol = 5))
